...
def save_data(tenant_id, database, table, data, case_id, case_id_based=True, view='records'):
    ...
    if case_id_based:
        ...
        try:
            ...
            logging.error(e)
            result["flag"]=False,
            result['data'] = {'reason':'Cannot update the database','error_msg':str(e)}
            return result
        result['flag']=True
        result['data']= data
        return result
...
def function_check(tenant_id, case_id, rule_list, data_tables, update_table_sources, return_vars):
    if len(rule_list)>0:
        ...
        if BR.changed_fields:
            ...
            BR.update_tables(updates, update_table_sources)
        logging.info(f"###### Return Data {return_data}")
...
def run_business_rule():
    try:
        ...
        logging.warning("Failed to Sstart ram and time calc")
