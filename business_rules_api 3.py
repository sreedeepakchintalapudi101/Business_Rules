...
def execute_business_rules():
    ...
    with zipkin_span(
        ...
        message, return_code, db_config['tenant_id'] = {'flag': False}, True, tenant_id
        rule_type, rule_name = data.get('link_type', "rule"), data.get('rule_name', "")
        return_data = handle_rule_execution(rule_type, data)
...
def rule_builder_data():
    ...
    trace_id = case_id if case_id else rule_id
    ...
    with zipkin_span(service_name='business_rules_api', span_name='rule_builder_data', transport_handler=http_transport, zipkin_attrs=attr, port=5010, sample_rate=0.5):
        ...
        if flag == 'save':
            ...
            try:
                ...
                logging.exception(e)
                return log_and_return("Error saving the rule to DB")
        elif flag == 'edit':
            try:
                business_db.update(table="rule_base", update=rule_base_table_dict, where={"rule_id": rule_id})
            except Exception as e:
                logging.exception(e)
                return log_and_return("Error updating the rule to DB")
        elif flag == 'fetch':
...
def get_data_sources(business_rules_db, case_id, column_name, master_data_columns=None, master=False):
    ...
    def fetch_data(db, table, case_id, master, columns_list=None):
        if master:
    ...
    data = {}
    if not sources:
        return data, sources  # Early return if no sources
    ...
    for database, tables in sources.items():
        ...
        for table in tables:
            ...
            data[table] = fetch_data(db, table, case_id, master, columns_list)
